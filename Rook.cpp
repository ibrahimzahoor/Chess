...
Rook::Rook(char clr):ChessPieces(clr)
{
	this->Symbol='R';
}

char Rook::Get_Piece_Color()
{
	return this->color;
}

char Rook::Get_Piece_Symbol()
{
	return this->Symbol;
}
	
bool Rook::MovePossible(ChessPieces *Board[8][8], int src_i, int src_j, int dest_i, int dest_j)
{
	if(dest_i < src_i && dest_j < src_j)
	{
		return false;
	}
